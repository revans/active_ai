...
module ActiveAI
  ...
  #   end
  #
  # Subclasses MUST implement `build_params` — returning the canonical params hash:
  #   { model:, max_tokens:, system:, messages:, skills:, source_files:, cacheable:, tools: }
  #
  # In practice, app code inherits ApplicationAgent (which implements build_params)
  ...
  class Base
        ...
        loop do
          ...
          if iterations > MAX_TOOL_ITERATIONS
            raise ToolLoopError,
              ...
              "possible infinite loop. Increase ActiveAI::Base::MAX_TOOL_ITERATIONS if the " \
    ...
    # Subclasses MUST implement. Returns the canonical params hash:
    #   { model:, max_tokens:, system:, messages:, skills:, source_files:, cacheable:, tools: }
    def build_params
      raise NotImplementedError, "#{self.class}#build_params is not implemented"
    end
